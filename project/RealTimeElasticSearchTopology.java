...
public class RealTimeElasticSearchTopology {
	...
	**/
	...
	Map<String, String> env = System.getenv();
        ...
        String accessToken = env.get("TWITTER_ACCESS_TOKEN");
        String accessTokenSecret = env.get("TWITTER_ACCESS_SECRET");
		...
		* using the code that looks like this:*/
		topology.newStream("tweets", spoutTweets)
			...
			.each(new Fields("text"), new SentenceBuilder(), new Fields("sentence")) //Adding sentenceBuilder here.
			.each(new Fields("sentence"), new DocumentBuilder(), new Fields("document"))
			.each(new Fields("document"), new ExtractDocumentInfo(), new Fields("id", "index", "type"))
			//.each(new Fields("sentence"), new Print("Sentence Field Neha:"))
			.groupBy(new Fields("index", "type", "id"))
			.persistentAggregate(stateFactory,  new Fields("document"), new TweetBuilder(), new Fields("tweet"))
			//.each(new Fields("tweet"), new Print("tweet field"))
			.parallelismHint(1) 
    ...
    public static void main(String[] args) throws Exception {
	   ...
	   ***/
	//Adding queries here
       String query1 = QueryBuilders.matchQuery("text", "you").buildAsBytes().toUtf8();
	   String query2 = QueryBuilders.fuzzyQuery("text", "hate").buildAsBytes().toUtf8();
	   String query3 = QueryBuilders.prefixQuery("text", "the").buildAsBytes().toUtf8();
	   
       String drpcResult;
       for (int i = 0; i < 5; i++) {
          drpcResult = drpc.execute("search_event", query1+" "+index_name+" "+index_type);
          System.out.println("DRPC RESULT: Query1: " + drpcResult);
		  drpcResult = drpc.execute("search_event", query2+" "+index_name+" "+index_type);
          System.out.println("DRPC RESULT:  Query2: " + drpcResult);
	      drpcResult = drpc.execute("search_event", query3+" "+index_name+" "+index_type);
          System.out.println("DRPC RESULT:  Query3: " + drpcResult);
          Thread.sleep(4000);
       }
